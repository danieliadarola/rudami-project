...
export default function NuevaSesion({ params }: { params: Promise<{ id: string }> }) {
  const { id } = use(params)
    ...
    <AppShell>
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="flex justify-between items-center mb-6">
          <div>
            <h1 className="text-2xl font-bold text-gray-900">RuDaMi Project</h1>
            <p className="text-gray-500 text-sm mt-0.5">
              Sesión clínica · {paciente ? `${paciente.nombre} ${paciente.apellidos}` : '...'}
            </p>
          </div>
          {copiloto && (
          ...
          <form onSubmit={handleSubmit} className="lg:col-span-3 space-y-4">
            ...
            <div className="bg-white rounded-xl border border-gray-200 p-5">
              ...
              <div className="mb-4">
                ...
                <textarea
                  ...
                  placeholder="Describe el estado actual, evolución desde la última visita, síntomas principales..."
                />
              </div>
            ...
            <div className="bg-white rounded-xl border border-gray-200 p-5">
              ...
              <div>
                ...
                <textarea
                  ...
                  rows={3}
                  className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500"
                  placeholder="Lasègue, Slump, Kemp, Phalen... y sus resultados"
                />
              </div>
            </div>
            ...
            <div className="bg-white rounded-xl border border-gray-200 p-5">
              ...
              <textarea
                ...
                rows={2}
                className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500"
                placeholder="Observaciones, ejercicios indicados, próximos pasos..."
              />
            </div>
            ...
            {loading && (
              <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
                <p className="text-blue-600 text-sm text-center">
                  Generando informe clínico completo con IA...
                </p>
              </div>
            )}
              ...
              <div className="p-4 max-h-screen overflow-y-auto">
                ...
                {!copiloto && !analizando && (
                  <div className="text-center py-8">
                    <p className="text-gray-400 text-sm">El copiloto se activará cuando empieces a escribir la anamnesis.</p>
                ...
                {copiloto && (
                  <div className="space-y-4">
                      ...
                      <div>
                        ...
                        <div className="flex flex-wrap gap-1">
                          {copiloto.diagnostico_diferencial.map((d, i) => (
                            <span key={i} className="text-xs bg-purple-50 text-purple-800 px-2 py-1 rounded-full border border-purple-100">
                              {d}
                            </span>
                          ))}
                        </div>
                      ...
                      <div>
                        ...
                        <div className="flex flex-wrap gap-1">
                          {copiloto.estructuras_implicadas.map((e, i) => (
                            <span key={i} className="text-xs bg-blue-50 text-blue-800 px-2 py-1 rounded-full border border-blue-100">
                              {e}
                            </span>
                          ))}
                        </div>
                    ...
                    {copiloto.tests_sugeridos.length > 0 && (
                      <div>
                        <p className="text-xs font-semibold text-gray-500 uppercase tracking-wide mb-2">
                          Tests ortopédicos sugeridos
                        </p>
                        <div className="flex flex-wrap gap-1">
                          {copiloto.tests_sugeridos.map((t, i) => (
                            <span key={i} className="text-xs bg-teal-50 text-teal-800 px-2 py-1 rounded-full border border-teal-100">
                              {t}
                            </span>
                          ))}
                        </div>
                      ...
                      <div>
                        ...
                        <div className="space-y-1">
                          {copiloto.preguntas_sugeridas.map((p, i) => (
                            <div key={i} className="text-xs bg-blue-50 text-blue-800 px-3 py-2 rounded-lg border border-blue-100">
                              {p}
                            </div>
                          ))}
                        </div>
