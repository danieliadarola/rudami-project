...
interface Cita {
  ...
  pacientes: { nombre: string; apellidos: string; telefono?: string }
}
...
export default function Calendario() {
  ...
  const cargar = async () => {
    ...
    const { data: citasData } = await supabase
      .from('citas').select('*, pacientes(nombre, apellidos, telefono)')
        ...
        <>
          ...
          <aside className="drawer">
            ...
            <div className="drawer-actions">
              {(() => {
                const tel = normalizarTelefono(sel.pacientes?.telefono)
                const msg = mensajeCita({ nombre: sel.pacientes?.nombre, fechaISO: sel.fecha_hora, tipo: selTipo?.nombre })
                return tel ? (
                  <button
                    onClick={() => window.open(whatsappLink(tel, msg), '_blank')}
                    style={{
                      display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 9,
                      width: '100%', height: 44, background: '#25D366', color: '#fff',
                      fontWeight: 600, fontSize: 14, border: 'none', borderRadius: 11,
                      cursor: 'pointer', fontFamily: 'inherit', boxShadow: '0 2px 10px rgba(37,211,102,.35)',
                    }}
                    onMouseEnter={e => (e.currentTarget.style.background = '#1eb558')}
                  ...
                  <button className="btn-line block" disabled title="El paciente no tiene teléfono">Sin teléfono para WhatsApp</button>
                )
              })()}
